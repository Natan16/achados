...
workon achados  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development achados npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop achados
    docker rm achados
    docker run --name achados -d --env-file /home/ubuntu/achados.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/achados:/dkdata \
        achados start_web.sh
}
...
echo_green "Welcome to achados's dev env"
